...
typedef struct {
  Local locals[UINT8_COUNT]; // Flat array of all locals that are in scope.
  int localCount;
  int scopeDepth;
} Compiler;
...
// Forward declarations that handle recursion in the grammar.
...
static void string(bool canAssign) {
  // Copy the string from the token. `+ 1` and `- 2` trim the leading and trailing quotes.
...
// Uses C99's designated initializer syntax
ParseRule rules[] = {
